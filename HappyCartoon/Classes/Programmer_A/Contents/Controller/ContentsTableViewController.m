...
//    注册cell
    ...
    if (!_placeHolderLabel)
        self.placeHolderLabel = [[PlaceHolderLabel alloc] initWithFrame:CGRectMake(0, 210, self.tableView.frame.size.width, 44)];
    _placeHolderLabel.text = @"读取目录失败,返回上层界面重试";
}
//初始化导航栏
-(void)initNavigation
{
...
-(void)JSON
{
    __unsafe_unretained ContentsTableViewController * selfController = self;
...
//            NSLog(@"请求数据失败");
        ...
        [_chapterListArray removeAllObjects];
...
//        设置简介model
        ...
        [selfController.comic setValuesForKeysWithDictionary:dicComic];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    myView.frame = CGRectMake(0, 0, self.view.frame.size.width, 10);
